...
def style_map_compat(styler, func, subset=None):
    """
    Compatibility helper for pandas Styler.

    New pandas uses Styler.map().
    Older pandas used Styler.applymap().
    This prevents Streamlit Cloud/local version mismatch crashes.
    """
...
st.sidebar.markdown("---")
st.sidebar.caption(
    "Empirical/Viewer/Explorer exclude A. baumannii/Acb complex by design."
)
...
if page == "Empirical Selector":
    ...
    st.caption(
        f"Overall • {source} "
        f"(Specimen = {specimen}) • A. baumannii excluded"
    )
    ...
    def agg_for_abx(g: pd.DataFrame) -> pd.Series:
        ...
        if has.sum() == 0:
            ...
            isolates_covered = float(
                sum(weights.get(o, 0.0) for o in g.loc[has, "Organism"].unique())
            )

        coverage = (
            isolates_covered / total_weight * 100.0
            if total_weight > 0
            else np.nan
        )
    ...
    styled = style_map_compat(
        styled,
        traffic_style_from_value,
    ...
    st.caption(
        "Recommended >70% • Borderline 50–70% • "
        "Not Recommended <50% based on Weighted Mean %"
    )
...
# =============================
    ...
    st.caption(
        "Pick an organism → see susceptibilities across specimens. "
        "Overall only. A. baumannii excluded."
    )

    df_spec = df_no_abaum[
        (df_no_abaum["Setting_cat"] == "Overall")
        & (df_no_abaum["Specimen"] != "All Specimen")
    ].copy()
    ...
    view_mode = st.selectbox(
        "View",
        ["Traffic-light matrix", "Ranked bar (combined)"],
        index=0,
    )
    ...
    if view_mode == "Traffic-light matrix":
        pivot_styled = pivot.style
        pivot_styled = style_map_compat(pivot_styled, traffic_style_from_value)
        pivot_styled = pivot_styled.set_table_styles(header_style())
    ...
    iso_by_spec = (
        org_df.dropna(subset=["Total_isolates"])
        .groupby("Specimen", as_index=False)["Total_isolates"]
        .max()
    )
    ...
    combined["Category"] = combined["Weighted Mean (%)"].apply(
        categorize_susceptibility
    )

    st.subheader("Combined view weighted across selected specimens")

    combined_styled = combined.style.format(
        {
            "Weighted Mean (%)": "{:.1f}",
        }
    )

    combined_styled = style_map_compat(
        combined_styled,
        traffic_style_from_value,
        subset=["Weighted Mean (%)"],
    )

    combined_styled = combined_styled.set_table_styles(header_style())
    ...
    chart_df = (
        combined.dropna(subset=["Weighted Mean (%)"])
        .set_index("Antibiotic")[["Weighted Mean (%)"]]
    )
    ...
    st.download_button(
        ...
        file_name=(
            re.sub(r"[^a-zA-Z0-9]+", "_", f"{org}_combined")
            .strip("_")
            .lower()
            + ".csv"
        ),
...
# =============================
    ...
    with c1:
        specimen = st.selectbox(
            "Specimen",
            ["All"] + sorted(out["Specimen"].unique().tolist()),
        )

    with c2:
        setting = st.selectbox(
            "Setting",
            ["All"] + sorted(out["Setting_cat"].unique().tolist()),
        )
    ...
    with c3:
        org_group = st.selectbox(
            ...
            + sorted(
                [
                    x
                    for x in out["Organism_group"].dropna().unique().tolist()
                    if x.strip()
                ]
            ),
        )
    ...
    with q3:
        only_tested = st.checkbox(
            "Only show rows with susceptibility data",
            value=False,
        )
    ...
    if sort_by == "Total_isolates (desc)":
        out = out.sort_values(
            ["Total_isolates", "Organism", "Antibiotic"],
            ascending=[False, True, True],
        )

    elif sort_by == "Susceptibility_% (desc)":
        out = out.sort_values(
            ["Susceptibility_%", "Organism", "Antibiotic"],
            ascending=[False, True, True],
        )
...
# =============================
    ...
    st.caption(
        "Dedicated view: A. baumannii / "
        "Acinetobacter calcoaceticus-baumannii complex only."
    )
    ...
    mode = st.radio(
        "View mode",
        [
            "Setting-based (All Specimen: ICU/Inpatient/Outpatient/Overall)",
            "Specimen-based (Overall only: Blood/Urine/Sputum/...)",
        ],
        index=0,
        horizontal=True,
    )
    ...
    if mode.startswith("Setting-based"):
        ...
        setting = st.selectbox(
            "Setting",
            sorted(view["Setting_cat"].unique().tolist()),
            index=0,
        )
        ...
        view = abaum[
            (abaum["Setting_cat"] == "Overall")
            & (abaum["Specimen"] != "All Specimen")
        ].copy()

        specimen = st.selectbox(
            "Specimen Overall only",
            sorted(view["Specimen"].unique().tolist()),
            index=0,
        )
    ...
    org = st.selectbox(
        "Organism label",
        sorted(view["Organism"].unique().tolist()),
        index=0,
    )

    show = view[
        view["Organism"] == org
    ].dropna(subset=["Susceptibility_%"]).copy()
    ...
    show_styled = show[cols_to_show].style.format(
        {
            "Susceptibility_%": "{:.1f}",
        }
    )

    show_styled = style_map_compat(
        show_styled,
        traffic_style_from_value,
        subset=["Susceptibility_%"],
    )

    show_styled = show_styled.set_table_styles(header_style())
